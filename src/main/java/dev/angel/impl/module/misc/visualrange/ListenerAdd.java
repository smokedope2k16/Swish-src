package dev.angel.impl.module.misc.visualrange;

import dev.angel.api.event.listener.ModuleListener;
import dev.angel.api.util.logging.Logger;
import dev.angel.impl.events.EntityWorldEvent;
import net.minecraft.entity.player.PlayerEntity;

@SuppressWarnings("ConstantConditions")
public class ListenerAdd extends ModuleListener<VisualRange, EntityWorldEvent.Add> {
    public ListenerAdd(VisualRange module) {
        super(module, EntityWorldEvent.Add.class);
    }

    @Override
    public void call(EntityWorldEvent.Add event) {
        if (event.getEntity() instanceof PlayerEntity player && !player.getName().getString().equals(mc.player.getName().getString())) {
            final String name = player.getName().getString();
            Logger.getLogger().log(String.format("%s has entered your visual range", name), player.getId());
        }
    }
}
